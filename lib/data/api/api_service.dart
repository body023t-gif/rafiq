...
class ApiService {
  ...
  static set dynamicToken(String? value) => SessionManager().accessToken = value;
  ...
  static set dynamicStudentId(String? value) => SessionManager().studentId = value;
  ...
  ApiService({
    ...
    Dio? dio,
            ...
            Dio(
              BaseOptions(
                ...
                responseType: ResponseType.bytes, // Force bytes response type for manual UTF-8 decoding
                headers: {
    ...
    _dio.interceptors.add(InterceptorsWrapper(
      onRequest: (options, handler) async {
        ...
        final maskedToken = _maskToken(options.headers['Authorization']?.toString());
  ...
  ApiException _mapDioException(DioException e) {
    ...
    if (data != null) {
      ...
      if (decoded is Map<String, dynamic>) {
        final msg = decoded['message'] ?? decoded['detail'] ?? decoded['title'] ?? decoded['error'];
        if (msg != null) {
    ...
    if (e.type == DioExceptionType.connectionError ||
        (e.type == DioExceptionType.unknown && e.error != null && e.error.toString().contains('SocketException'))) {
      return const ApiException('لا يوجد اتصال بالإنترنت.');
    }

    // 2. Map standard HTTP Status codes
    if (statusCode == 500) {
      return const ApiException('حدث خطأ في الخادم، حاول مرة أخرى لاحقاً.', statusCode: 500);
    }
    
    if (statusCode == 403) {
      return const ApiException('ليس لديك صلاحية لتنفيذ هذه العملية.', statusCode: 403);
    }
    if (statusCode == 404) {
    ...
    if (statusCode == 400) {
      return const ApiException('طلب غير صالح. يرجى التحقق من المدخلات.', statusCode: 400);
    }
    if (statusCode == 409) {
  ...
  String? _translateValidationError(String fieldName, String errorCode) {
    ...
    if (field == 'notes' || field == 'description') {
      if (code == 'RequiredField_Msg' || code.toLowerCase().contains('required') || code.isEmpty) {
        return 'يرجى إدخال سبب أو ملاحظات قبل إرسال الطلب.';
      }
    }
    if (field == 'appointmentdate') {
      if (code == 'RequiredField_Msg' || code.toLowerCase().contains('required')) {
